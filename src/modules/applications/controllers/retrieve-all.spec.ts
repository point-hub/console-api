...
describe('retrieve all applications', async () => {
  let app: Express
  beforeAll(async () => {
  ...
  it('retrieve success', async () => {
    ...
    const response = await request(app).get(`/v1/applications`)
    ...
    expect(response.body.data[0].name).toStrictEqual(applications.data[0].name)
    expect(isValid(new Date(response.body.data[0].created_date))).toBeTruthy()
    expect(response.body.data[1].name).toStrictEqual(applications.data[1].name)
    expect(response.body.data[2].name).toStrictEqual(applications.data[2].name)
  ...
  it('sort data in ascending order', async () => {
    ...
    applicationFactory.sequence(data)
    await applicationFactory.createMany(3)

    const response = await request(app).get(`/v1/applications`).query({
      sort: 'name',
    })
  ...
  it('navigate pagination', async () => {
    ...
    expect(response.body.data[0].name).toStrictEqual(applications.data[2].name)
