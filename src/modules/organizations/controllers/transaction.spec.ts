...
describe('organization use transaction', async () => {
  let app: Express
  beforeAll(async () => {
  ...
  it('transaction aborted when create failed', async () => {
    ...
    await request(app).post('/v1/organizations/transaction').send(data)
  ...
  it('transaction aborted when update failed', async () => {
    ...
    // expect recorded data
    const organizationRecords = await DatabaseTestUtil.retrieveAll('organizations')
    expect(organizationRecords.data.length).toStrictEqual(3)
  })
  it('transaction aborted when update many failed', async () => {
    ...
    // expect recorded data
    const organizationRecords = await DatabaseTestUtil.retrieveAll('organizations')
    expect(organizationRecords.data.length).toStrictEqual(4)
    expect(organizationRecords.data[0].name).toStrictEqual('John 5')
  })
